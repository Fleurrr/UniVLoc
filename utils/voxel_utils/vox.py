...
from utils.geometry import ssc_to_homo
...
class Vox_util(object):
    def __init__(self, Z, Y, X, scene_centroid, bounds, pad=None, assert_cube=False):
        self.XMIN, self.XMAX, self.ZMIN, self.ZMAX, self.YMIN, self.YMAX, = bounds[0],  bounds[1],  bounds[2],  bounds[3],  bounds[4],  bounds[5]     
        # B, D = list(scene_centroid.shape)
        self.Z, self.Y, self.X = Z, Y, X

        scene_centroid = scene_centroid.detach().cpu().numpy()
        x_centroid, y_centroid, z_centroid = scene_centroid[0], scene_centroid[1], scene_centroid[2]
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        # for i in range(5):
        #     canvas = np.ones((240, 360, 1))
        #     plt.figure(1)
        #     plt.scatter(x[i].clone().detach().cpu().numpy(), y[i].clone().detach().cpu().numpy(), c=np.ones_like(x[i].clone().cpu().numpy())-1, cmap='jet', alpha=0.1, s=1)
        #     plt.xlim(0, 240)
        #     plt.ylim(0, 360)
        #     plt.show()
        #     plt.savefig("/mnt/workspace/proj"+str(i)+".jpg")
