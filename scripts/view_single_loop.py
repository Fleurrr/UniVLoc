...
for folder_path in os.listdir(folder_paths):
    csv_file = os.path.join(folder_paths, folder_path + '/tum_vehicle_lidar_odometry_pose.csv')
    ...
    i = i+1
    if os.path.exists(folder_paths + '/' + folder_path + '/graph.txt'):
      graph_path = os.path.join(folder_paths, folder_path + '/graph.txt')
f = open(graph_path)
lines = f.readlines()
time2index = dict()
