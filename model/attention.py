...
from model.ms_deform_attn import MSDeformAttn, MSDeformAttn3D
...
class VanillaSelfAttention(nn.Module):
    def __init__(self, dim=128, dropout=0.1, spatial_shapes=(100, 100)):
        ...
        self.Z, self.X = spatial_shapes[0], spatial_shapes[1]
...
class VanillaCrossAttention(nn.Module):
    def __init__(self, dim=128, dropout=0.1, spatial_shapes=(100, 100)):
        ...
        self.dim = dim 
        self.dropout = nn.Dropout(dropout)
        self.deformable_attention = MSDeformAttn(d_model=dim, n_levels=1, n_heads=4, n_points=40)
        self.output_proj = nn.Linear(dim, dim)

        self.Z, self.X = spatial_shapes[0], spatial_shapes[1]

    def forward(self, query, key, query_pos=None):
        '''
        query: (B, N, C)
        '''
        inp_residual = query.clone()
        ...
        input_spatial_shapes[:] = self.Z
        input_level_start_index = query.new_zeros([1,]).long()
        queries = self.deformable_attention(query, reference_points, key, 
            input_spatial_shapes.detach(), input_level_start_index.detach())
...
class SpatialCrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, query_pos=None, reference_points_cam=None, spatial_shapes=None, bev_mask=None, level_start_index=None, reference_points_ego=None):
        '''
        query: (B, N, C)
        key: (S, M, B, C)
        reference_points_cam: (S, B, N, D, 2), in 0-1, (w, h)
        bev_mask: (S, B, N, D)
        '''
        inp_residual = query.clone()
        ...
        max_len = max([len(each) for each in indexes]) # find co-visible points indexes for each camera, squeeze z-axis

        queries_rebatch = query.new_zeros(
            [B, S, max_len, self.dim])
        reference_points_rebatch = reference_points_cam.new_zeros(
            [B, S, max_len, D, 2]) # query must align with reference points

        # reference_points_ego_rebatch = reference_points_ego.new_zeros(
        #     [B, S, max_len, D, 2]) # query must align with reference points      
        ...
        for j in range(B):
            for i, reference_points_per_img in enumerate(reference_points_cam):
                ...
                reference_points_rebatch[j, i, :len(index_query_per_img)] = reference_points_per_img[j, index_query_per_img] # (B, S, max_len, Y, 2)

        # for j in range(B):
        #     for i, reference_points_per_img in enumerate(reference_points_ego):
        #         index_query_per_img = indexes[i]
        #         reference_points_ego_rebatch[j, i, :len(index_query_per_img)] = reference_points_per_img[j, index_query_per_img]
