...
class RPEMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, embed_qk=None, key_weights=None, key_masks=None, attention_factors=None):
        ...
        """
        q = rearrange(self.proj_q(input_q), 'b t n (h c) -> b h t n c', h=self.num_heads)
        k = rearrange(self.proj_k(input_k), 'b t m (h c) -> b h t m c', h=self.num_heads)
        v = rearrange(self.proj_v(input_v), 'b t m (h c) -> b h t m c', h=self.num_heads)
        if embed_qk is not None:
            p = rearrange(self.proj_p(embed_qk), 'b t n m (h c) -> b h t n m c', h=self.num_heads)
            attention_scores_p = torch.einsum('bhtnc,bhtnmc->bhtnm', q, p)
        
        attention_scores_e = torch.einsum('bhtnc,bhtmc->bhtnm', q, k)
        if embed_qk is not None:
            attention_scores = (attention_scores_e + attention_scores_p) / self.d_model_per_head ** 0.5
        else:
            attention_scores = attention_scores_e / self.d_model_per_head ** 0.5
        ...
        hidden_states = rearrange(hidden_states, 'b h t n c -> b t n (h c)')
...
class RPERadiusTransformerLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(RPERadiusTransformerLayer, self).__init__()
