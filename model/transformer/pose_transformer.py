...
from model.transformer.output_layer import AttentionOutput
...
class PoseAttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        super(PoseAttentionLayer, self).__init__()
        self.attention = MultiHeadAttention(d_model, num_heads, dropout=dropout)
        self.linear = nn.Linear(d_model, d_model)
        self.dropout = build_dropout_layer(dropout)
        self.norm = nn.LayerNorm(d_model)
...
class TransformerPoseDecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(TransformerPoseDecoderLayer, self).__init__()
        self.self_attention = PoseAttentionLayer(d_model, num_heads, dropout=dropout)
        self.cross_attention = PoseAttentionLayer(d_model, num_heads, dropout=dropout)
        self.output = AttentionOutput(d_model, dropout=dropout, activation_fn=activation_fn)

    def forward(self, input_states, memory_states_s, memory_states_r, embeddings=None, input_masks=None, memory_masks=None):
        if embeddings is not None:
            memory_states_s =  memory_states_s + embeddings
            memory_states_r =  memory_states_r + embeddings
        hidden_states, attention_scores = self.self_attention(input_states, input_states, input_states, memory_masks=input_masks)
        hidden_states, attention_scores = self.cross_attention(hidden_states, memory_states_s, memory_states_r, memory_masks=memory_masks)
        output_states = self.output(hidden_states)
        return output_states, attention_scores
...
class TransformerPoseDecoder(nn.Module):
    def __init__(self, d_model, num_heads, num_layers, pose_dim, query_num, dropout=None, activation_fn='ReLU'):
        super(TransformerPoseDecoder, self).__init__()
        self.num_layers = num_layers
        self.pose_dim = pose_dim
        self.bev_proj = nn.Linear(d_model, d_model)
        layers = []
        for _ in range(num_layers):
            layers.append(TransformerPoseDecoderLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.pose = nn.Parameter(0.1*torch.randn(1, d_model))
        self.bev_pos = nn.Parameter(0.1*torch.randn(query_num, d_model))
        self.pose_query_to_pose = MLP(d_model, d_model // 2, pose_dim, num_layers)
    ...
    def forward(self, q_feats, s_feats):
        ...
        """
        B = q_feats.shape[0]
        pose = self.pose.clone().unsqueeze(0).repeat(B, 1, 1)
        bev_pos = self.bev_pos.clone().unsqueeze(0).repeat(B, 1, 1)
        q_feats = self.bev_proj(q_feats)
        s_feats = self.bev_proj(s_feats)
        for i in range(self.num_layers):
            pose, _ = self.layers[i](pose, q_feats, s_feats, embeddings=bev_pos)
        pose_decoded = self.pose_query_to_pose(pose).squeeze(1)
        return pose_decoded