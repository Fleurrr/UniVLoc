...
class TransformerPlaceDecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(TransformerPlaceDecoderLayer, self).__init__()
        self.self_attention = AttentionLayer(d_model, num_heads, dropout=dropout)
        self.cross_attention = AttentionLayer(d_model, num_heads, dropout=dropout)
        self.output = AttentionOutput(d_model, dropout=dropout, activation_fn=activation_fn)

    def forward(self, input_states, memory_states, embeddings=None, input_masks=None, memory_masks=None):
        if embeddings is not None:
            memory_states =  memory_states + embeddings
        hidden_states, attention_scores = self.self_attention(input_states, input_states, memory_masks=input_masks)
        hidden_states, attention_scores = self.cross_attention(hidden_states, memory_states, memory_masks=memory_masks)
        output_states = self.output(hidden_states)
        return output_states, attention_scores
...
class TransformerPlaceDecoder(nn.Module):
    def __init__(self, d_model, num_heads, num_layers, desc_dim, query_num, dropout=None, activation_fn='ReLU'):
        super(TransformerPlaceDecoder, self).__init__()
        self.num_layers = num_layers
        self.desc_dim = desc_dim
        self.bev_proj = nn.Linear(d_model, d_model)
        layers = []
        for _ in range(num_layers):
            layers.append(TransformerPlaceDecoderLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.desc = nn.Parameter(0.1*torch.randn(1, d_model))
        self.bev_pos = nn.Parameter(0.1*torch.randn(query_num, d_model))
        self.desc_query_to_desc = MLP(d_model, d_model * 2, desc_dim, num_layers)
    ...
    def forward(self, q_feats):
        ...
        """
        B = q_feats.shape[0]
        desc = self.desc.unsqueeze(0).repeat(B, 1, 1)
        bev_pos = self.bev_pos.unsqueeze(0).repeat(B, 1, 1)
        q_feats = self.bev_proj(q_feats)
        for i in range(self.num_layers):
            desc, _ = self.layers[i](desc, q_feats, embeddings=bev_pos)
        desc_decoded = self.desc_query_to_desc(desc).squeeze(1)
        return desc_decoded