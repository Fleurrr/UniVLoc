...
def spherical_projection(dataset_type, imgs, f, prespheredir) :
    ...
    if not os.path.exists(prespheredir + "sphere_points.npy") and not os.path.exists(prespheredir + "pix.npy"):
    ...
    for cam_id in range(len(imgs)):
        # idx, valid = XYZtoRC(dataset_type, '/map-hl/gary.huang/public_dataset/NCLT/cam_params/image_meta.pkl', points, imgs[cam_id], cam_id)
        idx, valid = XYZtoRC(dataset_type, '/map-hl/gary.huang/public_dataset/Oxford/dataset/image_meta.pkl', points, imgs[cam_id], cam_id)
...
if __name__ == "__main__":
    ...
    for name in os.listdir(args["dir"]):
        ...
        imagePaths = sorted(list(paths.list_images(os.path.join(args["dir"], name))))
        ...
        # stitcher = Stitcher('nclt', '/map-hl/gary.huang/public_dataset/NCLT/cam_params/image_meta.pkl')
        # result = stitcher.stitch(images, "/map-hl/gary.huang/public_dataset/NCLT/paro/")
        # result = cv2.rotate(result, cv2.ROTATE_180)
        # cv2.imwrite("/map-hl/gary.huang/public_dataset/NCLT/paro/" + name + ".jpg", result)

        stitcher = Stitcher('oxford', '/map-hl/gary.huang/public_dataset/Oxford/dataset/image_meta.pkl')
        result = stitcher.stitch(images, "/map-hl/gary.huang/public_dataset/Oxford/paro/")
        result = cv2.rotate(result, cv2.ROTATE_180)
        cv2.imwrite("/map-hl/gary.huang/public_dataset/Oxford/paro/" + name + ".jpg", result)
