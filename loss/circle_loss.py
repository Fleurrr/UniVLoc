...
def neg_only_weighted_circle_loss(
    neg_masks,
    feat_dists,
    neg_margin,
    neg_optimal,
    ...
    # get anchors that have both positive and negative pairs
    row_masks = torch.gt(neg_masks.sum(-1), 0).detach()
    col_masks = torch.gt(neg_masks.sum(-2), 0).detach()
    ...
    loss_row = F.softplus(loss_neg_row) / log_scale
    loss_col = F.softplus(loss_neg_col) / log_scale
...
class WeightedCircleLoss(nn.Module):
    ...
    def forward(self, pos_masks=None, neg_masks=None, feat_dists=None, pos_scales=None, neg_scales=None, neg_only=False):
        if not neg_only:
            ...
            return neg_only_weighted_circle_loss(
                neg_masks,
                feat_dists,
                self.neg_margin,
                self.neg_optimal,
